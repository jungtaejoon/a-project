<template>
  <div class="submenu collapse-item" :class="{ 'active': isActived }">
    ...
    <transition name="collapsed-fade" :css="false" @before-appear="before" @appear="enter" @appear-cancel="cancel"
                @before-enter="before"
...
export default {
  ...
  mounted () {
    function activate (self) {
      if (self.$refs.box) {
        self.isActived = self.$refs.box.querySelectorAll('a.active').length >= 1
      }
    }
